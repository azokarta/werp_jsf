...
@ManagedBean(name = "wfaclistBean", eager = true)
@ViewScoped
public class WfacList {
	private final static String transaction_code = "WFACLIST";
	private final static Long transaction_id = 475L;
	...
	public void init() {
		...
		try {
			...
			outputTable.searchModel.setMarkedType(Contract.MT_CUT_OFF_PRICE);
			outputTable.searchModel.setMtConfirmed(Contract.MT_CONFIRMED_FALSE);
...
//				outputTable.searchModel.setBranch_id(0L);
	...
	public void loadBranch2() {
		...
		branch_list = loadBranchListByUserBranch(outputTable.getSearchModel().getBukrs(), branchId, BusinessArea.AREA_ALL_EXCEPT_SERVICE);
		RequestContext reqCtx = RequestContext.getCurrentInstance();
