...
@ManagedBean(name = "logInvoiceCrudBean")
@ViewScoped
public class CrudBean implements Serializable {
	...
	public void initBean(String mode, String type) {
		this.mode = mode;
		this.type = type;
		...
		if (!GeneralUtil.isAjaxRequest()) {
			if (mode.equals("create")) {
				...
				} else if (type.equals("write_off_doc")) {
					PermissionController.canRead(userData, writeOffDocTransactionId);
					invoice.setType_id(Invoice.TYPE_WRITEOFF_DOC);
				}
	...
	private void loadInvoiceItems() {
		...
		if (invoice != null) {
			...
			String cond = "invoice_id = " + invoice.getId();
			if (invoice.getType_id().equals(Invoice.TYPE_WRITEOFF_DOC)) {
				//cond += " AND quantity > done_quantity ";
			}
			List<InvoiceItem> temp = d.findAll(cond);
	...
	public void Save() {
		try {
			...
			if (invoice.getType_id().equals(Invoice.TYPE_POSTING)) {
				...
				GeneralUtil.doRedirect("/logistics/invoice/View.xhtml?id=" + invoice.getId());
			}
	...
	List<ParentDocsClass> parentDocs = new ArrayList<CrudBean.ParentDocsClass>();
	Map<Long, Integer> selectedParentDocMap = new HashMap<Long, Integer>();
	List<ParentDocsClass> loadedParentDocs = new ArrayList<CrudBean.ParentDocsClass>();
	...
	private void loadDocsForParent() {
		parentDocs = new ArrayList<CrudBean.ParentDocsClass>();
		if (invoice.getType_id() == Invoice.TYPE_POSTING) {
			OrderDao d = (OrderDao) appContext.getContext().getBean("orderDao");
			for (Order o : d.findAll(" status_id = " + Order.STATUS_OPENED + " ORDER BY id DESC ")) {
				ParentDocsClass pdc = new ParentDocsClass();
				pdc.setDocumentName("Заказ");
				pdc.setId(o.getId());
				Staff stf = stfMap.get(o.getCreated_by());
				if (stf != null) {
					pdc.setCreatorName(stf.getLF());
				}
				pdc.setContext(Order.CONTEXT);
				loadedParentDocs.add(pdc);
			}
		} else if (invoice.getType_id().equals(Invoice.TYPE_SEND)) {
			InvoiceDao d = (InvoiceDao) appContext.getContext().getBean("invoiceDao");
			for (Invoice i : d.findAll(" type_id = " + Invoice.TYPE_SEND)) {
				ParentDocsClass pdc = new ParentDocsClass();
				pdc.setDocumentName("Заказ");
				pdc.setId(i.getId());
	...
	public void assignParentDoc() {
		if (selectedParentDoc != null && !selectedParentDocMap.containsKey(selectedParentDoc.getId())) {
			if (selectedParentDoc.getContext().equals(Order.CONTEXT)) {
				OrderMatnrDao d = (OrderMatnrDao) appContext.getContext().getBean("orderMatnrDao");
				List<OrderMatnr> omList = d.findAll("order_id = " + selectedParentDoc.getId());
				invoiceItems = new ArrayList<InvoiceItem>();
				parentDocs = new ArrayList<CrudBean.ParentDocsClass>();
				...
				if (invoice.getType_id().equals(Invoice.TYPE_POSTING_IN)) {
					parentDocs = new ArrayList<CrudBean.ParentDocsClass>();
					invoiceItems = new ArrayList<InvoiceItem>();
