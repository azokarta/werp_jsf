...
@ManagedBean(name = "hrEducationBean")
@ViewScoped
public class HrEducation implements Serializable {
	...
	private StaffEducation selected = new StaffEducation();
	...
	private void Update(StaffEducation se) {
		getEduService().update(se);
	}

	public void Delete(StaffEducation se) {
		System.out.println(se.getClass().toString());
		try{
			getEduService().delete(se);
			GeneralUtil.addSuccessMessage("Удалено успешно!");
		}catch(DAOException e){
			GeneralUtil.addErrorMessage(e.getMessage());
		}
	}
	...
	/**************************************/
	private StaffEducationService getEduService() {
		return (StaffEducationService) appContext.getContext().getBean(
