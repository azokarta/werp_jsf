...
@ManagedBean(name = "fa02Bean", eager = true)
@ViewScoped
public class Fa02 implements Serializable{
	private static final long serialVersionUID = 1L;
	private final static String transaction_code = "FA02";
	private final static Long transaction_id = (long) 2;
	...
	public void init() {
		...
		try {				
			...
			PermissionController.canRead(userData,Fa02.transaction_id);
			...
			if (userData.isNew_fi_doc())
			{
				TransactionDao transactionDao = (TransactionDao) appContext.getContext().getBean("transactionDao"); 
				new_fi_doc = true;
				trans_url = transactionDao.find(userData.getTrans_id()).getUrl();
				userData.setNew_fi_doc(false);
				userData.setTrans_id(0L);
			}
	...
	private String trans_message = "Документ сохранен";
	...
	{
		try{
			PermissionController.canWrite(userData, Fa02.transaction_id);
			int count = 0;
			...
			{	
				...
				count = bkpfDao.dynamicCountBkpf("storno = 0 and blart <> 'ST' and awkey = "+wa_awkey);
				//System.out.println(count);
				if (count == 0 && p_bkpf.getStorno() == 0)
				{
					p_bkpf = financeService.cancelFA02(p_bkpf, l_bseg, userData,transaction_code);
					if (p_bkpf.getStorno()==1)
					{
						p_bkpf_storno = bkpfDao.findStornoSingleBkpf(p_bkpf.getBelnr(), p_bkpf.getGjahr(),p_bkpf.getBukrs());
					}
				...
				else if (p_bkpf.getBlart().equals("G2"))
				{
					p_bkpf = financeService.cancelFA02(p_bkpf, l_bseg, userData,transaction_code);
					if (p_bkpf.getStorno()==1)
					{
				...
				{
					throw new DAOException("You can't cancel the document");
				}
			}
			else
			{
