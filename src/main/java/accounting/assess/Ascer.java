...
@ManagedBean(name = "ascerBean", eager = true)
@ViewScoped
public class Ascer implements Serializable{ 
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L; 
	private final static String transaction_code = "ASCER";
	private final static Long transaction_id = (long) 638;
   ...
   //**********************************************************************    
	...
	public void to_save() throws IOException{
		try { 
			PermissionController.canWrite(userData, Ascer.transaction_id);
			...
			p_bsegDebet.setHkont("33100001");
			p_bsegDebet.setBschl("28");
			p_bsegDebet.setShkzg("S");					
			p_bsegDebet.setLifnr(p_bkpf.getCustomer_id());
			if (p_bsegDebet.getLifnr() == null)
			{
			...
			userData.setTrans_id(Ascer.transaction_id);
	...
	public void changeBukrs()
	{
		...
		if (!(p_bkpf.getBukrs().equals(null)|| p_bkpf.getBukrs().equals("0"))){
			Hkont wa = hkontDao.findByIds(p_bkpf.getBukrs(), "70100013");			
			if (wa!=null) hkont_list.add(wa);			
		}
