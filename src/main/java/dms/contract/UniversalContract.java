...
@ManagedBean(name = "universalConBean", eager = true)
@ViewScoped
public class UniversalContract implements Serializable {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 2841819316961515511L;
	private final static String transaction_code = "UC";
	private final static Long transaction_id = (long) 496;
	...
	public void assignFoundEmployee() {
		...
		if (selectedStaff != null && selectedStaff.getStaff_id() != null) {
			...
			} else if (p_selected_position_id == 3) {
				org.springframework.beans.BeanUtils.copyProperties(selectedStaff, p_manager);
				p_fioManager = selectedStaff.getFullFIO();
			} else if (p_selected_position_id == 10) {
				org.springframework.beans.BeanUtils.copyProperties(selectedStaff, p_director);
				p_fioDirector = selectedStaff.getFullFIO();
		...
		reqCtx.update("form:manager");
		reqCtx.update("form:director");
	...
	public void to_save() {
		...
		try {
			...
			if (p_contract != null) {
				...
				double dmbtr = 0;
				List<Payroll> pl = createPayrollList();
				
				if (getContractService().createUniversalContract(p_contract,
						p_branch, pl, userData, transaction_id, transaction_code, p_currency,
	...
	private List<Payroll> createPayrollList() {
		List<Payroll> pl = new ArrayList<>();
	
		if (!Validation.isEmptyLong(p_contract.getDealer()) && (p_dealerPremi > 0)) {
			Payroll p = new Payroll();			
//			select p.staff_id,p.bukrs,p.dmbtr,p.waers,p.branch_id,p.contract_number,p.position_id from payroll p
			p.setStaff_id(p_contract.getDealer());
			p.setBukrs(p_contract.getBukrs());
			p.setDmbtr(p_dealerPremi);
			p.setWaers(p_currency);
			p.setBranch_id(p_contract.getBranch_id());
			p.setContract_number(null);
			p.setPosition_id(4L);
			pl.add(p);
		}
		
		if ((p_director != null)  && (p_directorPremi > 0)) {
			Payroll p = new Payroll();			
//			select p.staff_id,p.bukrs,p.dmbtr,p.waers,p.branch_id,p.contract_number,p.position_id from payroll p
			p.setStaff_id(p_director.getStaff_id());
			p.setBukrs(p_contract.getBukrs());
			p.setDmbtr(p_directorPremi);
			p.setWaers(p_currency);
			p.setBranch_id(p_contract.getBranch_id());
			p.setContract_number(null);
			p.setPosition_id(10L);
			pl.add(p);
		}
		
		if ((p_manager != null)  && (p_managerPremi > 0)) {
			Payroll p = new Payroll();			
//			select p.staff_id,p.bukrs,p.dmbtr,p.waers,p.branch_id,p.contract_number,p.position_id from payroll p
			p.setStaff_id(p_manager.getStaff_id());
			p.setBukrs(p_contract.getBukrs());
			p.setDmbtr(p_managerPremi);
			p.setWaers(p_currency);
			p.setBranch_id(p_contract.getBranch_id());
			p.setContract_number(null);
			p.setPosition_id(3L);
	...
	public void updateAverage() {
		...
		if (p_contract.getPayment_schedule() > 0) {
			average = (double) ((long) remain / p_contract
					.getPayment_schedule());
			remainder = (double) (remain % p_contract.getPayment_schedule());
		} else {
			average = 0;
			remainder = 0;
		}
