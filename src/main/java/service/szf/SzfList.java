...
@ManagedBean(name = "szflistBean", eager = true)
@ViewScoped
public class SzfList implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = -8217624195938375496L;
	private final static String transaction_code = "SZFLIST";
	private final static Long transaction_id = (long) 196;
	...
	public void init() {
		...
		try {
			...
			toMainPage();
			PermissionController.canRead(userData, SzfList.transaction_id);
			if (userData.isMain()) {
	...
	private void checkAccessLevel() {
		...
		for (UserRole wa_ur : urL) {
			...
			if (wa_ur.getRoleId() == 24 || wa_ur.getRoleId() == 30 || wa_ur.getRoleId() == 29) {
				isCoordinator = true;
			}
		}
	...
	public void to_search() {
		try {
			...
			sfoList = getSfServ().getZFList(p_bukrs, p_branch_id, p_month,
					p_list_type, p_conNum, p_tovSN, p_list_cat);
	...
	private ServFilterService getSfServ() {
		return (ServFilterService) appContext.getContext().getBean(
				"servFilterService");
	}

	public List<ServFilter> sfL = new ArrayList<ServFilter>();
	...
	private void calcPlan() {
		...
		for (ServFilter sf : sfL) {
			...
			if (p_overdue) {
				...
				if (GeneralUtil.getFirstDateOfMonth(sf.getF2_date_next())
						.getTime() < GeneralUtil.getFirstDateOfMonth(p_month)
						.getTime()
						|| (isInMonth(sf.getF2_date())
								&& sf.getF2_date_prev() != null && GeneralUtil
								.calcAgeinMonths(sf.getF2_date(),
										sf.getF2_date_prev()) > sf.getF2_mt())) {
					fc++;
					f2Q++;
				}
				if (GeneralUtil.getFirstDateOfMonth(sf.getF3_date_next())
						.getTime() < GeneralUtil.getFirstDateOfMonth(p_month)
						.getTime()
						|| (isInMonth(sf.getF3_date())
								&& sf.getF3_date_prev() != null && GeneralUtil
								.calcAgeinMonths(sf.getF3_date(),
										sf.getF3_date_prev()) > sf.getF3_mt())) {
					fc++;
					f3Q++;
				}
				if (GeneralUtil.getFirstDateOfMonth(sf.getF4_date_next())
						.getTime() < GeneralUtil.getFirstDateOfMonth(p_month)
						.getTime()
						|| (isInMonth(sf.getF4_date())
								&& sf.getF4_date_prev() != null && GeneralUtil
								.calcAgeinMonths(sf.getF4_date(),
										sf.getF4_date_prev()) > sf.getF4_mt())) {
					mc++;
					f4Q++;
				}
				if (GeneralUtil.getFirstDateOfMonth(sf.getF5_date_next())
						.getTime() < GeneralUtil.getFirstDateOfMonth(p_month)
						.getTime()
						|| (isInMonth(sf.getF5_date())
								&& sf.getF5_date_prev() != null && GeneralUtil
								.calcAgeinMonths(sf.getF5_date(),
										sf.getF5_date_prev()) > sf.getF5_mt())) {
					fc++;
					f5Q++;
				}
			} else {
				...
				if (isInMonth(sf.getF2_date_next())
						|| (isInMonth(sf.getF2_date())
								&& (sf.getF2_date_prev() != null) && GeneralUtil
								.calcAgeinMonths(sf.getF2_date(),
										sf.getF2_date_prev()) == sf.getF2_mt())) {
				...
				if (isInMonth(sf.getF3_date_next())
						|| (isInMonth(sf.getF3_date())
								&& (sf.getF3_date_prev() != null) && GeneralUtil
								.calcAgeinMonths(sf.getF3_date(),
										sf.getF3_date_prev()) == sf.getF3_mt())) {
				...
				if (isInMonth(sf.getF4_date_next())
						|| (isInMonth(sf.getF4_date())
								&& (sf.getF4_date_prev() != null) && GeneralUtil
								.calcAgeinMonths(sf.getF4_date(),
										sf.getF4_date_prev()) == sf.getF4_mt())) {
				...
				if (isInMonth(sf.getF5_date_next())
						|| (isInMonth(sf.getF5_date())
								&& (sf.getF5_date_prev() != null) && GeneralUtil
								.calcAgeinMonths(sf.getF5_date(),
										sf.getF5_date_prev()) == sf.getF5_mt())) {
	...
	public void calcDone() {
		List<ServiceTable> servL = getServiceDao().findAllInCurrentMonthByType(
				p_branch_id, p_month, ServiceTable.TYPE_FILTERS);
		...
		for (ServiceTable s : servL) {
			if (s.getServ_type() == ServiceTable.TYPE_FILTERS) {
				List<ServicePos> spL = getServPosDao().findAllByServID(
						s.getId());
				ServFilter sf = getSfServ().getSFByContractId(
						s.getContract_id());
				int fc = 0;
				int mc = 0;
				...
				if (sf != null) {
					for (ServicePos sp : spL) {
						...
						if (!Validation.isEmptyLong(sp.getMatnr_id())
							...
							case 3: {
								// if (sf.getF1_sid() == s.getId()) {
								if ((sf.getF3_date_next().getTime() > dl
										.getTime())
										&& (isInMonth(sf.getF3_date())
												&& (sf.getF3_date_prev() != null) && GeneralUtil
												.calcAgeinMonths(
														sf.getF3_date(),
														sf.getF3_date_prev()) <= sf
												.getF3_mt())) {
									fc++;
									isOverdue = false;
								}
							...
							case 4: {
								// if (sf.getF1_sid() == s.getId()) {
								if ((sf.getF4_date_next().getTime() > dl
										.getTime())
										&& (isInMonth(sf.getF4_date())
												&& (sf.getF4_date_prev() != null) && GeneralUtil
												.calcAgeinMonths(
														sf.getF4_date(),
														sf.getF4_date_prev()) <= sf
												.getF4_mt())) {
									mc++;
									isOverdue = false;
								}
							...
							case 5: {
								// if (sf.getF1_sid() == s.getId()) {
								if ((sf.getF5_date_next().getTime() > dl
										.getTime())
										&& (isInMonth(sf.getF5_date())
												&& (sf.getF5_date_prev() != null) && GeneralUtil
												.calcAgeinMonths(
														sf.getF5_date(),
														sf.getF5_date_prev()) <= sf
												.getF5_mt())) {
									fc++;
									isOverdue = false;
								}
	...
	public void savePlan() {
		try {
			byte p_type;
			if (p_overdue)
				p_type = SalePlan.TYPE_OVERDUE_CEBILON;
			else
				p_type = SalePlan.TYPE_CURRENT_CEBILON;
