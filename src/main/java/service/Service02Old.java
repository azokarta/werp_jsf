...
@ManagedBean(name = "service02OldBean", eager = true)
@ViewScoped
public class Service02Old {
	/**
	 * 
	 */
		...
		try {
			...
			if (Validation.isEmptyString(p_service.getBukrs()))
				...
				MatnrDao mDao = (MatnrDao) appContext.getContext().getBean(
						"matnrDao");
				Matnr matnr = mDao.find(ct.getMatnr());
				...
				spl_old = new ArrayList<ServicePos>(sp_l);
				int i = 1;
				sum_wrbtr = 0;
				sum_dmbtr = 0;
	...
	public boolean spChanged(List<ServicePos> spL1, List<ServicePos> spL2) {
		...
		for (int i = 0; i < spL1.size(); i++) {
			if (spL1.get(i).hashCode() != spL2.get(i).hashCode()) return true;
		}		
		return res;
	}
	...
	public void loadPriceList() {
		...
		for (PriceList wa_priceList2 : p_priceListF4Bean.getPriceListByCountry(
				p_service.getBukrs(), p_branch.getCountry_id())) {
...
//					+ wa_priceList2.getWaers());
	...
	public void loadByCategory() {
		st_l = new ArrayList<Service02Old.ServType_Loc>();
		...
		Comparator<ServType_Loc> c = new Comparator<Service02Old.ServType_Loc>() {
			@Override
	...
	public void assignFoundEmployee() {
		...
		if (selectedStaff != null
				&& !Validation.isEmptyLong(selectedStaff.getStaff_id())) {
			...
			SalaryDao sDao = (SalaryDao) appContext.getContext().getBean(
					"salaryDao");
//			List<Salary> sL = sDao.findByStaffId(selectedStaff.getStaff_id());
//			for (Salary s : sL) {
//				selectedStaff.setPosition_id(s.getPosition_id());
//				if ((p_service.getCategory() == ContractType.TOVARCAT_VACUUM_CLEANER && selectedStaff
//						.getPosition_id() == Staff.POS_MASTER_VAC)
//						|| (p_service.getCategory() == ContractType.TOVARCAT_WATER_FILTER && (selectedStaff
//								.getPosition_id() == Staff.POS_MASTER_WAT
//								|| selectedStaff.getPosition_id() == Staff.POS_MASTER_FILTER || selectedStaff
//								.getPosition_id() == Staff.POS_MASTER_FILTER_OVERDUE))) {
//					p_service.setMaster_id(selectedStaff.getStaff_id());
//					p_master = Clone.cloneStaff(selectedStaff);
//					doCreateON();
//				} else if (selectedStaff.getPosition_id() == Staff.POS_OPER_FILTER
//						|| selectedStaff.getPosition_id() == Staff.POS_OPER_VAC) {
...
//			}
	...
	public void calcRow(int index) {
		...
		if (sp2 != null && sp2.getQuantity() != null) {
			...
			servPosListTable.get(pos).getServPos()
					.setSumm(sp2.getQuantity() * sp2.getMatnr_price());
	...
	public void removeEmptyRows() {
		...
		for (int i = 0; i < servPosListTable.size(); i++) {
			...
			if (servPosListTable.get(i) != null && servPosListTable.get(i).isWarranty())
			...
			if (p_service.getServ_type() == 2) {
				...
				for (ServFilterPremis sfp : sfpl) {
					if (sfp.getFc() == fc && sfp.getMc() == mc) {
						...
						p_service.setPayment_due(sfp.getTotal());
				...
				p_service.setPayment_due(sum_wrbtr);
				p_service.setCurrency(p_currency);
	...
	public List<Service02Old.ServType_Loc> st_l = new ArrayList<Service02Old.ServType_Loc>();

	public List<Service02Old.ServType_Loc> getSt_l() {
		return st_l;
	}
	...
	public void setSt_l(List<Service02Old.ServType_Loc> st_l) {
		this.st_l = st_l;
	}
	...
	private void loadContractModel() {
		// String cond = " ";
		String cond = " and tovar_serial <> null and contract_status_id <> 3";
	...
	public void assignSelectedMatnr() {
		...
		double rate = 1;
		/*
		 * if (pl2.getWaers() != null && pl2.getWaers().length() > 0) { rate =
		 * getCurrencyRate(pl2.getWaers(), p_currency);
		 * System.out.println(pl2.getWaers() + " vs " + p_currency + " rate: "+
		 * rate); }
		 */
		...
		if (p_service.getServ_type() != ServiceTable.TYPE_FILTERS) {
			...
			if (p_service.getBranch_id() == 73 
					|| p_service.getBranch_id() == 74
					|| p_service.getBranch_id() == 75)
		...
		String inf = "";
		if (rate != 1) {
			// inf = "Initial Price: " + pl2.getPrice() + " " + pl2.getWaers() +
			// " | ";
		...
		MatnrWarDao mwDao = (MatnrWarDao) appContext.getContext().getBean(
				"matnrWarDao");
		MatnrWar mw = mwDao.findByMatnr(selectedMatnr.getMatnr().getMatnr());
...
//			System.out.println("Cancel Filter Service!");
	...
	private void createService(ServiceService ss) {
		...
		if (docancel) p_service.setPaid(0);
