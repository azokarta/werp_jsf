...
@ManagedBean(name = "frep4Bean", eager = true)
@ViewScoped
public class Frep4 implements Serializable{ 
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private final static String transaction_code = "FREP4";
	private final static Long transaction_id = (long) 374;
	...
	public void init() { 
		...
		{ 
			...
			PermissionController.canRead(userData,Frep4.transaction_id);
...
//  			p_searchTable.setBudat_to(date_to.getTime());
	  		...
	  		if (p_searchTable.blart.equals("0"))
	  		{
	  			dynamicWhereClause = dynamicWhereClause + " and b.blart in ('CP','CF','DP')";
	  		}
	  		else dynamicWhereClause = dynamicWhereClause + " and b.blart in ('"+p_searchTable.blart+"')";
	  		
	  		
  			RfcolDao rfcolDao = (RfcolDao) appContext.getContext().getBean("rfcolDao");
  			List<Object[]> results = new ArrayList<Object[]>();
  			results = rfcolDao.dynamicFrep4Result(dynamicWhereClause,userData.getU_language());
...
//  					dynamicWhereClause,  GeneralUtil.getSQLDate(cal_budat_from), GeneralUtil.getSQLDate(cal_budat_to),1,p_searchTable.getP_status());
	...
	public void updateBranch()
	{
		try{
			...
			//24 *НАЧАЛЬНИК СЕРВИСНОГО ОТДЕЛА*
			Long ba = 0L;
			Long br = 0L;
			p_searchTable.getL_branch().clear();
			
			if (userData.getL_ra_map().get(24L)!=null && userData.getL_ra_map().get(24L).size()>0)
			{
	...
	public void getDetail(Long a_branch_id,String a_waers,String a_hkont)
	{
		try 
		{
			...
			l_detailTable.clear();
  			...
  			RfcolDao rfcolDao = (RfcolDao) appContext.getContext().getBean("rfcolDao");
  			List<Object[]> results = new ArrayList<Object[]>();
  			results = rfcolDao.dynamicFrep4Detail(dynamicWhereClause,userData.getU_language());
  			for (Object[] wa_result:results)
  			{
  				...
  				if (wa_result[10]!=null)
					try {
						wa_out.setBldat(formatter.parse(String.valueOf(wa_result[10])));
					} catch (ParseException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
  				//if (wa_result[10]!=null) wa_out.setBldat(wa_result[10]));
  				if (wa_result[11]!=null) wa_out.setOld_sn(Long.parseLong(String.valueOf(wa_result[11])));
