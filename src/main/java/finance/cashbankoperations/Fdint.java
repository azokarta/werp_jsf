...
@ManagedBean(name = "fdintBean", eager = true)
@ViewScoped
public class Fdint implements Serializable{ 
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private final static String transaction_code = "FDINT";
	private final static Long transaction_id = (long) 117;
   ...
   //********************************************************************** 
	...
	public void assignSelectedCurrency(){
		...
		try{
			if (p_bkpf.getBukrs()==null || p_bkpf.getBukrs().length()<4)
			{
	...
	public class BsegTypeTable {
		...
		public BsegTypeTable(int a_index){ 
			this.index = a_index;
			this.t_bseg.setShkzg("H");
			this.t_bseg.setBschl("50");
		}
	...
	public void to_save() throws IOException{
		try { 
			...
			PermissionController.canWrite(userData, Fdint.transaction_id);
			...
			if (selectedCustomer.getIin_bin().equals("1000")) p_bsegDebet.setHkont("12101000");
			else if (selectedCustomer.getIin_bin().equals("2000")) p_bsegDebet.setHkont("12102000");
			else if (selectedCustomer.getIin_bin().equals("3000")) p_bsegDebet.setHkont("12103000");
			//else if (selectedCustomer.getIin_bin().equals("0001")) p_bsegDebet.setHkont("12104000");
			else if (selectedCustomer.getIin_bin().equals("5000")) p_bsegDebet.setHkont("12105000");
			else if (selectedCustomer.getIin_bin().equals("6000")) p_bsegDebet.setHkont("12106000");
			else if (selectedCustomer.getIin_bin().equals("7000")) p_bsegDebet.setHkont("12107000");
			else if (selectedCustomer.getIin_bin().equals("8000")) p_bsegDebet.setHkont("12108000");
			p_bsegDebet.setBukrs(p_bkpf.getBukrs());
			p_bsegDebet.setGjahr(p_bkpf.getGjahr());
			p_bsegDebet.setBschl("1");
			p_bsegDebet.setShkzg("S");
			p_bsegDebet.setLifnr(selectedCustomer.getId());
			...
			userData.setTrans_id(Fdint.transaction_id);
			
//			Long newDocBelnr = financeService.createAccountReceivableDocs(p_bkpf, l_bsegFinal, selectedBranch);

			Long newDocBelnr = financeService.createFdint(p_bkpf, l_bsegFinal);
	...
	{
		selectedCustomer = null;
		l_as = new ArrayList<CashBankAccountStatement>();
		assignFoundCustomer();
		to_search_customer();
		p_bkpf.setWaers("");
		p_bkpf.setKursf(0);
