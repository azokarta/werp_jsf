...
@ManagedBean(name = "faicfpBean", eager = true)
@ViewScoped
public class Faicfp implements Serializable{
	private static final long serialVersionUID = 1L;
	private final static String transaction_code = "FAICFP";
	private final static Long transaction_id = (long) 127;
	...
	public void init() {
		try
		{
			...
			PermissionController.canRead(userData,Faicfp.transaction_id);
	...
	public void assignSelectedHkont()
	{
		try
		{
			...
			{
				throw new DAOException("No need to select in the 2nd position GL account");
			}
	...
	public void to_save()
	{
		try
		{			
			PermissionController.canWrite(userData, Faicfp.transaction_id);
			...
			try {
				...
				userData.setTrans_id(Faicfp.transaction_id);
	...
	public void to_check()
	{
		...
		{			
			...
			p_bkpf.setTcode(Faicfp.transaction_code);
	...
	public void clearAndAddBseg()
	{
		...
		l_bseg.get(1).getP_bseg().setHkont("10200001");
		RequestContext reqCtx = RequestContext.getCurrentInstance();
