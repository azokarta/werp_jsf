...
public class ServFilterServiceImpl implements ServFilterService {
	...
	public List<ServFilterOutput> getZFList(String in_bukrs, Long in_branchID, Date in_date, 
			int in_list_type, Long in_con_num, String in_tovSN, int in_cat) throws DAOException {
		List<ServFilterOutput> sfoL = new ArrayList<ServFilterOutput>();
		
		try {
			if (!Validation.isEmptyString(in_tovSN)) {
				in_tovSN = in_tovSN.trim();
				sfoL = sfDao.findAllBySN(in_bukrs, in_con_num, in_tovSN);
			} else {
				switch (in_list_type) {
				case 1:
					sfoL = sfDao.findAllByBranch(in_bukrs, in_branchID, in_cat);
					break;

				case 2:
					sfoL = sfDao.findAllCurrentByDate(in_bukrs, in_branchID, in_date);
					break;

				case 3:
					sfoL = sfDao.findAllOverdueByDate(in_bukrs, in_branchID, in_date);
					break;
	...
	@Override
	public List<ServFilter> getZFListForPlan(String in_bukrs, Long in_branchID, Date in_date, boolean overdue) throws DAOException {
		List<ServFilter> sfL = new ArrayList<ServFilter>();
		try {
			if (overdue) 
				sfL = sfDao.findAllForOverduePlanByDate(in_bukrs, in_branchID, in_date);
			else
				sfL = sfDao.findAllForCurrentPlanByDate(in_bukrs, in_branchID, in_date);
	...
	@Override 
	public ServFilter getSFByContractId(Long inConId) throws DAOException {
		try {
			return sfDao.findByContractID(inConId);
		} catch(Exception ex) {
			throw new DAOException(ex.getMessage());
		}
	}
	...
	@Override
	public boolean updateSF(ServFilter in_sf, User userData, Long trId) throws DAOException {
		try {
